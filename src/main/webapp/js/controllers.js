...
module.controller('KeywordsController', ['$scope', 'AnalyzerService', 'HtmlCleanerService', '$timeout', '$compile', '$window',  function($scope, AnalyzerService, HtmlCleanerService,$timeout,$compile, $window) {
    var view = createExtractorView($scope,$timeout,$compile, $window);
    var model = new ExtractorModel(AnalyzerService, HtmlCleanerService);
    ...
    view.onAnalyze = function(){
        var data = view.getAnalyzeData();
        if(data){
            $scope.previewMode = false;
            if(!view.infoPopupOpen){
                view.openInfoPopup();
                view.manuallyInsertedText = true;
            }
            model.getAnalysis(data, view.afterAnalyzing);
        }
    };
    view.onClear = function(){
        view.clear();
    };
    view.onToggleEdition = function(){
        view.previewMode=!view.previewMode;
    };
    view.onReset = function(){
        view.reset();
    };
    view.onWgetClean = function(){
        var phrase = view.wgetClean();
        if(phrase){
            view.openInfoPopup();
            model.getHtmlText(phrase, view.afterWGetClean);
        }

    };
    view.onOpenInfoPopup = function(){
        view.openInfoPopup();
    };
    view.onCloseInfoPopup = function(){
        view.closeInfoPopup();
    };
    view.onCancelInfoPopup = function(){
        view.cancelInfoPopup();
    };
    view.onLimitMainText = function(){
        view.limitMainText();
    };
    view.onActionOnPreview = function(){
        view.actionOnPreview();
    };
}]);